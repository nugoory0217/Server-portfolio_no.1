...
- 결론: 동일한 정적 페이지에 대해 동시 접속 수를 50에서 1000으로 증가시킨 결과, 서버의 최대 처리량은 약 12k requests/sec 수준에서 포화되었으며, 이후에는 처리량 증가 없이 응답 지연(latency)만 증가하는 전형적인 saturation 패턴이 관찰되었습니다. 특히 Waiting 및 Connect 시간이 크게 증가하여 서버 리소스 및 요청 처리 큐에서 병목이 발생했음을 확인할 수 있었습니다.<br><br>

2) PHP+CPU 부하 페이지
- 위의 정적 페이지와의 차이점을 중심으로 확인 해보겠습니다.
  1. 정적 페이지에서는 Failed Requests가 없으나, CPU 부하 페이지에서는 발생했습니다.
  2. Connection times 부분에서 정적 페이지 에서는 Processing과 Waiting의 차이가 크지만 CPU 부하 페이지 에서는 차이가 거의 없습니다.
  3. Percentile 부분에서 정적 페이지 에서는 과부하 시 전체적으로 지연 시간이 다 올라갔지만 CPU 부하 페이지 에서는 90%부터 갑자기 지연시간이 올라감(과부하 전 스크린샷)을 확인 할수 있었습니다.
   
- 위의 3가지 차이점은 전형적인 CPU 부하 시 의 특성을 보여줍니다.<br> 
  1번 Failed Requests의 사유를 보면 전부 Length 임을 확인 할수 있습니다. Length는 ab가 기대한 응답 body 길이와 실제 길이가 다를 경우에 카운트 되는 부분입니다. ab가 기대한 응답 body 길이는 첫번째 정상 응답의 Content-Length를 기준으로 삼기 때문에 CPU의 처리 시간의 다름으로 인해 생긴 문제라고 추정 할수 있습니다.<br><br>
  2번 Connection times의 경우 Processing = Waiting + Content Transfer 로 구성되어 있습니다. 정적 페이지에서는 Processing과 Waiting이 차이가 크지만 CPU 과부하 페이지에서는 차이가 적습니다. Content Transfer는 응답의 첫 바이트를 받은 이후부터 마지막 바이트를 받을 때 까지의 시간이기 때문에 CPU 연산이 늘어져서 생기는 지연은 전부 Waiting입니다. 즉, Processing에서 Waiting이 차지하는 비율이 크다는건 CPU 연산의 지연을 추정 할 수 있습니다.<br><br>
  3번 Percentile 부분에서 정적 페이지의 경우 특별히 튀는 구간 없이 전체적으로 지연이 일어났지만 CPU 부하페이지에서는 비록 심한 과부하 이전이지만 90% 이상 부터 갑자기 지연이 심해짐을 확인 할수 있었습니다. 이처럼 뒷부분이 심하게 느려지는 현상을 Tail latency라 하는데 Tail latency의 원인은 실제 자동차 교통 체증의 "유령체증"과 비슷합니다. 앞 부분에서 미세하게 정체가 일어날 경우 그에 반응한 뒷차의 미세한 반응이 추가되고 누적되어 가장 끝에 있는 차량은 젤 처음 정체를 일으킨 차랑의 지체 시간에 비해 몇배의 정체가 생기는 현상입니다. CPU 과부하 페이지의 경우 CPU의 계산을 강제하는 페이지이기 때문에 정적 페이지에 비해 CPU의 계산이 반드시 필요하고, 그 계산은 캐시 미스, branch misprediction, 스케줄링 등으로 결과가 나오기 까지의 시간이 매번 같을 수가 없습니다. 그로 인해 미세하게 들쭉 날쭉한 시간이 걸리게 되고 이는 곧 자동차 교통체증인 "유령체증"과 같이 뒷부분이 느려지게 되는 결과를 가져옵니다.<br><br>
- 결론: Static 페이지와 CPU 연산 페이지 모두 과부하 시 RPS 정체와 latency 증가라는 공통된 현상을 보였으나, Static 페이지는 단순 큐잉에 의해 느려졌을 뿐 안정적으로 처리된 반면, CPU 연산 페이지는 요청 당 CPU 점유로 인해 처리 지연과 실패가 발생했습니다. 특히 CPU 페이지에서는 Waiting과 Processing 시간이 거의 동일하고, p90 이상의 Tail latency가 증가하여 내부 연산 병목이 발생했음을 확인할 수 있었습니다.
  
3) PHP+DB 부하 페이지
- 위의 CPU 부하 페이지와의 차이점을 중심으로 확인 해보겠습니다.
  1. 과부하 발생 후 Requests per second가 크게 늘었습니다.
  2. Percentile 부분에서 Tail latency가 CPU에서는 특정 구간에서 크게 튀었지만 DB부하 시에는 전체적으로 느려졌습니다.
   
- 위의 2가지 차이점은 CPU 부하 시 와는 다른 특성을 보여줍니다.<br> 
  1번 Requests per second가 CPU 부하때는 크게 늘지 않았는데 DB부하 시에는 크게 늘은 이유는 DB가 병목일 때는 DB쿼리만 막히고 PHP,Nginx 등 DB보다 앞단의 서버들은 여유가 있기 때문에 생긴 증상으로 추정 됩니다. 갑자기 늘어는 RPS는 CPU연산 하고는 관계가 없겠구나 하고 DB쪽을 장애의 원인으로 추정할 근거가 됩니다.<br><br>
  2번 Percentile 부분의 Tail latency 역시 마찬가지 입니다. CPU의 경우 연산의 결과가 매번 일정한 시간에 나오지 않기 때문에 Tail latency가 튀는 구간이 보였지만 DB과부하는 CPU 연산이 필요 없이 일정하게 지연만 걸었기 때문에 모두가 줄서서 기다리는 상황으로 보이고 특정 구간에 특이점 없이 전체적으로 느려진 것으로 보입니다.<br><br>
- 결론: Apache Bench 결과만으로는 CPU 병목과 DB 병목의 수치적 결과가 상당 부분 유사하게 나타났습니다.
이는 AB가 병목의 원인을 구분하기보다는, 요청 지연과 Queueing이라는 결과를 중심으로 측정하기 때문입니다.
다만 DB 병목의 경우 초당 처리량(RPS)이 크게 증가하고, 전체 지연 분포가 균등하게 상승하는 특징을 보였습니다.
